...
# Page configuration
st.set_page_config(
    page_title="AI Phone Call Assistant",
    page_icon="📞",
    layout="wide",
...
def load_workflow_data():
    workflow_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "workflow_ai_phone_call.json")
...
def main():
    ...
    <div class="main-header">
        <h1>📞 AI Phone Call Assistant</h1>
        <p style="font-size: 1.2rem; margin-top: 0.5rem;">Google Calendar Booking Assistant 24/7</p>
    </div>
    """, unsafe_allow_html=True)
    
    # Load workflow
    workflow_data = load_workflow_data()
    workflow_name = workflow_data.get('name', 'AI Phone Call Assistant')
    ...
    with col2:
        ...
        st.markdown(f'''<div class="description-content">The AI Phone Call Assistant is an intelligent automation system that handles phone calls and seamlessly integrates with Google Calendar for instant scheduling. This 24/7 solution ensures smarter meetings and instant scheduling by leveraging AI to manage appointments, reduce no-shows, and optimize your calendar automatically.</div>''', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
        
        # Description 2: Key Features
        st.markdown('<div class="description-box">', unsafe_allow_html=True)
        st.markdown('<div class="description-title">🚀 Key Features</div>', unsafe_allow_html=True)
        st.markdown(f'''<div class="description-content">• AI-powered phone call handling
...
• Real-time availability checking</div>''', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
